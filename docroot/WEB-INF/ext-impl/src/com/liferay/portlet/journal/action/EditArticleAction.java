...
import com.liferay.portal.journal.action.png.PngEncoderExtend;
...
public class EditArticleAction extends PortletAction {
	...
	protected Object[] updateArticle(ActionRequest actionRequest)
		...
		if (Validator.isNotNull(structureId)) {
			...
			Object[] contentAndImages = getCustomeContentAndImages(
					ddmStructure, locale, serviceContext);
	...
	private Object[] getCustomeContentAndImages(
		...
		Map <String, byte[]> imagesModified = new HashMap<String, byte[]>();
		
		for(Entry<String, byte[]> image : images.entrySet()){
			
			try {
				
				InputStream is = new BufferedInputStream(new ByteArrayInputStream(image.getValue()));
				String mimeType = URLConnection.guessContentTypeFromStream(is);
				is.close();
				
				byte[] bytesOfCompressImage = null;
				
				if(mimeType != null && mimeType.equalsIgnoreCase("image/jpg") || mimeType.equalsIgnoreCase("image/jpeg")){
					String limitValue = "0.9";
					if (PropsUtil.contains(JPG_COMPRESSION_LEVEL)){
						limitValue = PropsUtil.get(JPG_COMPRESSION_LEVEL);
					}
					
					String imageSuffix = mimeType.split("/")[1]; // can not directly put jpg - has jpeg too
					System.out.println("image type : " + imageSuffix);
					if(imageSuffix != null && limitValue != null){
						BufferedImage bufferedImage = createImageFromBytes(image.getValue());
						bytesOfCompressImage = compressImage( bufferedImage , imageSuffix, Float.parseFloat(limitValue));
						if(bytesOfCompressImage == null){
							bytesOfCompressImage = image.getValue();
							System.out.println("bytesOfCompressImage is null - image not compressed properly - keeping the original image");
						}
					}
					else{
						bytesOfCompressImage = image.getValue();
						System.out.println("imageSuffix or compression-limit value is null - image not compressed properly - keeping the original image");
					}
				}
				else if(mimeType != null && mimeType.equalsIgnoreCase("image/png")){
					
					System.out.println("image type : " + "png");
					bytesOfCompressImage = getCompressPngImage(image.getValue());
					
					if(bytesOfCompressImage != null){
						imagesModified.put(image.getKey(), bytesOfCompressImage);
					}
					else{
						bytesOfCompressImage = image.getValue();
						System.out.println("bytesOfCompressPngImage is null - Png image not compressed properly - keeping the original image");
					}
				}
				else{
					bytesOfCompressImage = image.getValue();
					System.out.println("No mimeType - Image not compressed");
				}
		...
		return new Object[] {
				JournalConverterUtil.getContent(ddmStructure, fields), imagesModified
			};
		
	}
	
	private byte[] compressImage(BufferedImage image,String imageSuffix,
			float quality) throws IOException
	{
		byte[] compressedImage = null;
		try {
			// Get a ImageWriter for jpeg format.
			Iterator<ImageWriter> writers = ImageIO.getImageWritersBySuffix(imageSuffix);
			if (!writers.hasNext())
			...
			compressedImage = bos.toByteArray();
		} 
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private byte[] getCompressPngImage(byte[] image){
		
		byte[] finalCompressedBytes = null;
		
		try {
			PngEncoderExtend encoder = new PngEncoderExtend();
			//encoder.setCompressed(true);
			encoder.setCompressed(false); // make more compress
			BufferedImage bufferedImage = createImageFromBytes(image);
			OutputStream osTemp = new ByteArrayOutputStream(32768);
			OutputStream oss = encoder.write(bufferedImage, osTemp);
			osTemp.close();
			ByteArrayOutputStream bos = (ByteArrayOutputStream)oss;
			finalCompressedBytes = bos.toByteArray();
			oss.close();
		} 
		catch (IOException e) {
			e.printStackTrace();
	...
	private static final String JPG_COMPRESSION_LEVEL = "com.liferay.image.jpg.compress.level"; //0.9
